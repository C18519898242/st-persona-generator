...
表情图统一使用 `896 × 1280`、`7:10` 竖版画布，与当前模板的默认表情框比例一致。

- 必须包含头部、肩部和少量上半身。
- 双眼中心应位于图片高度约 `30%`，允许上下偏差不超过 `5%`。
- 六张表情图应保持相同镜头距离、发型、服装、背景和光线。
...
六张表情分别生成，只替换 `[目标表情]`。第一张 `exp_calm.jpg` 的目标表情必须为“自然平静”。
...
最终画布必须为 896 × 1280 px，宽高比 7:10，竖版半身肖像。
画面包含完整头发、头部、双肩和少量上半身。
双眼中心位于画面高度约 30%，允许上下偏差不超过 5%。
脸部水平居中，头顶保留安全距离，肩部不能被不自然截断。
使用与其余五张表情完全相同的镜头距离、发型、服装、背景、光线和色彩。
背景为干净、均匀的暖米白，柔和人像光。

只改变表情和与表情直接相关的细微头部动作。
