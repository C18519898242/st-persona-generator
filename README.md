...
表情图与 bootstrap 参考图都是**统一的上半身半身像**（头+双肩+上衣主体），不是大头特写，也不是全身立绘。统一使用 `896 × 1280`、`7:10` 竖版画布，与当前模板的默认表情框比例一致。

- 半身像构图：完整头发、头部、双肩与上半身；下沿约到**胸口中下部/上腹**；上衣主体清楚可见。
- 头部（含发型）约占画面高度 `35%–45%`（避免头过大）；双眼中心约在 `28%` 一带；脸部水平居中。
...
最终画布必须为 896 × 1280 px，宽高比 7:10，竖版上半身半身像。
画面包含完整头发、头部、双肩与上半身，下沿约到胸口中下部；上衣主体可见。
头部约占画高 35%–45%，双眼中心约 28%，禁止大头特写占满画幅。
