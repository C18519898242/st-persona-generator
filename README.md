...
表情图与 bootstrap 头像都是**统一的证件照式头肩特写**（脸+双肩，少露躯干），不是全身立绘，也不是露到腰腹的松散半身照。统一使用 `896 × 1280`、`7:10` 竖版画布，与当前模板的默认表情框比例一致。

- 标准头像构图：完整头发、头部与双肩；下沿约到**锁骨或胸口中上部**；只露少量领口与肩线。
- 头部（含发型）约占画面高度 `50%–60%`；双眼中心约在 `34%` 一带；脸部水平居中。
- **六张表情必须是同一个人**：同一镜头距离、机位高度、发型、服装、背景和光线；只改表情。
- 禁止手部遮挡五官、夸张漫画表情、大幅转头，禁止换装/换发型，禁止腰线以下入画。
- 第一张 `exp_calm.jpg` 使用自然平静表情，必须清楚显示五官、发型和耳饰，因为它同时用于“面部与发型”细节，并作为其余五张的身份锚点。
- 六张表情**只**锚定人物头像做「表情编辑」，不挂全身像、不挂已生成的 `exp_calm`（避免错误锚点扩散）。
- 构图必须与参考头像一致：双肩与上衣领口可见，禁止比头像更近的大脸特写。
- 表情落盘时**不做**二次头肩强裁（强裁会把正确头像裁成大脸特写）；bootstrap 生成偏长半身头像时仍可轻度收紧。
...
最终画布必须为 896 × 1280 px，宽高比 7:10，竖版标准头像特写。
画面包含完整头发、头部与双肩，下沿约到锁骨或胸口上沿；不要腰腹、不要全身或大半身。
双眼中心位于画面高度约 30%，允许上下偏差不超过 5%。
...
`exp_calm.jpg` 同时用于页面的“面部与发型”细节，并作为其余五张表情的身份锚点，因此这一张尤其需要：
