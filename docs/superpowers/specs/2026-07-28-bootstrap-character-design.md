...
2. 生成/跳过头像（参考 = 全部 `sample/*`）  
3. 生成/跳过全身像（参考 = `sample/*`，若头像文件已存在则**追加**头像以加强脸一致）  
4. 打印结果与下一步
...
**头像：** 上半身半身像（非大头特写、非全身）；完整头发/头/双肩/上衣主体；下沿约到胸口中下部；头部约占画高 35–45%；眼位约 28%；平静表情；上装与工作装一致或不冲突；落盘前仅轻度裁掉过长下方。

**全身像：** 单人正面全身立绘；头到鞋完整；人物约占画高 86–91%；**默认工作装**；自然站姿。
